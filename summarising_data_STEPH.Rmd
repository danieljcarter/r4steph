...
```{r}
#--- Get difference from overall mean
...
```{r}
#--- Get difference from grouped mean
sdg %>% group_by(reg) %>% 
  mutate(diff.gdp = gdp - mean(gdp, na.rm = T)) %>% 
  select(country, gdp, diff.gdp) %>% head()
```

The possibilities of mutate() are pretty endless. Here are a few use cases. Make sure you understand what each line of code is doing.
...
> EXERCISE: Get the cumulative mean incidence of TB as you go from low GDP to high GDP. (hint: filter, arrange, mutate, cummean)
...
We can use the data we have summarised directly in a call to ggplot() to graph our newly summarised data. We are going to construct a concentration curve using some summarised data. Check that you can follow what each line does (and remeber you can build this line by line!)
